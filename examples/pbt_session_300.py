...
def session(
    ...
    def load_agent(env, agent_data, num_gpus):
        ...
        population_params = population_params._replace(population_size = int(population_params.population_size/num_gpus))
    ...
    # # make directories to save subsequent agents
    # output_dirs = []
    # for i in range(self.params.population_size):
    #     single_path = output_dir, ('agent_' + str(epoch_circle) + '_' + str(input_dict['gpu']) + '_' + str(i))
    #     os.makedirs(os.path.join(single_path))
    #     output_dirs.append(single_path)
    ...
    output_dir = os.path.join(output_dir, ('generation_' + str((num_gpus*1000 + epoch_circle)))
    ...
    if self_play:
        agents[0].save_weights_fast(
            os.path.join(output_dir, "weights"), mean_reward)
    else:
