...
def session(
            ...
            n_parallel: int = 1080,  ## has to be multiple of population_size
            n_parallel_eval:int = 2560, ## has to be multiple of population_size
            n_train_steps: int = 4,
            n_sim_steps: int = 2,
            epochs: int = 3500,
            eval_freq: int = 1002,
    ...
    if self_play:
        with gin.config_scope('agent_0'):
            ...
            if restore_weights != None:
                ...
                # opt_states = multiplace_agent(states)
    ...
    for epoch in range(pbt_epochs):
        ...
        if self_play:
            if epoch in saver_epochs:
                agents[0].save_weights(os.path.join(output_dir, 'weights'), 'Rainbow_diversity_at_{}_epochs'.format(epoch))
